...
int robot_peut_avancer(Terrain t, Robot r) {
  int x1, y1;
  position_devant(r, &x1, &y1);
  if ( est_case_libre(t,x1,y1) ) {
  	return 1;
  }
        switch(orient(r)) {
        case Nord : if (ordonnee(r)==0) return 1; break;
        case Est  : if (abscisse(r)==largeur(t)-1) return 1; break;
        case Sud  : if (ordonnee(r)==hauteur(t)-1) return 1; break;
        case Ouest: if (abscisse(r)==0) return 1; break;
        }	
